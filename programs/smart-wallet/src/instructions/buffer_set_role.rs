...
pub enum BufferRoles {
    /// Unknown buffer role.
    Unknown = 0,
    /// The admin role to with permission to set the [InstructionBuffer::authority] or [InstructionBuffer::executor].
    Admin = 1,
...
pub fn handler(ctx: Context<SetBufferRole>, role: BufferRoles, role_key: Pubkey) -> Result<()> {
    ...
    match role {
        ...
        BufferRoles::Writer => {
            old_role_key = buffer.authority;
            buffer.authority = role_key;
        }
        BufferRoles::Executer => {
            old_role_key = buffer.executor;
            buffer.executor = role_key;
        }
